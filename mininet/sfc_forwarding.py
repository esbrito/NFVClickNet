    ...
    def deploy_pair(self, sfc_id, _src, _dst, _dst_ip, first,
        ...
        if(_src[:1] == 's') and (_dst[:1] == 's'):
            ...
            # Test to see if it is the fisr switch connected to src
                ...
                in_port = self.edges_ports["s%s" % dpid][first]['in_port']
                out_port = self.edges_ports[
                           "s%s" % dpid]["s%s" % dst]['out_port']
                    ...
                    # port of the connection of the other interface
                        ...
                        self.edges_ports[
                             ...
                             {link_dst: {'in_port': int(inp),
                              'out_port': int(outp)}})
        _file.close
        # Makes elist so it can do the graph used to enable inet
        _file = open('topology.txt', 'r')
        reg = re.compile(r'-eth([0-9]+):([\w]+)-eth[0-9]+')
        regSwitch = re.compile('(s[0-9]+) lo')
        for line in _file:
            if "lo:" not in line:
                continue
            ...
            for conn in reg.findall(connections):
                self.elist.append((refnode, conn[1])) if (conn[1], refnode) \
                    not in self.elist else None
        _file.close

    def enable_inet_pop(self, pops):
        self.logger.debug("<enable_inet_pop> called")
        # transform the inner keys of the nested dicts in a list
        for dpid in pops:
            datapath = self.dp_dict[dpid]
            parser = datapath.ofproto_parser
            dpid_pop = datapath.id
            # internet will always be at s1 -- according to mininet
            path = nx.shortest_path(self.graph, "h%s" % dpid_pop, "nat0")

            pop_ip = "10.0.0.%s" % path[0][1:]
            ip_server = "143.54.85.8"

            pop_inport = self.edges_ports[
                         's%s' % dpid]['h%s' % dpid]['in_port']
            # blocking arp on the second interface -> to avoid arp storm
            match = parser.OFPMatch(eth_type=ether.ETH_TYPE_ARP,
                                    arp_spa=pop_ip, in_port=pop_inport)
            action = []
            self.add_flow(datapath=datapath, priority=5000, match=match,
                          actions=action)

            match = parser.OFPMatch(eth_type=ether.ETH_TYPE_ARP,
                                    arp_tpa=pop_ip, in_port=pop_inport)
            action = []
            self.add_flow(datapath=datapath, priority=5000,
                          match=match, actions=action)

            for switch, i in zip(path, xrange(0, len(path))):
                if i == 0:
                    continue
                if i == len(path) - 1:
                    break
                ...
                dpid = datapath.id
                parser = datapath.ofproto_parser
                match = parser.OFPMatch(eth_type=0x0800, ipv4_src=pop_ip,
                                        ipv4_dst=ip_server)
                action = [parser.OFPActionOutput(int(
                                self.edges_ports[
                                     switch][path[i+1]]['out_port']))]
                self.add_flow(datapath=datapath, priority=4096, match=match,
                              actions=action)
                # backward
                match = parser.OFPMatch(eth_type=0x0800, ipv4_src=ip_server,
                                        ipv4_dst=pop_ip)
                if path[i-1] == path[0]:
                    output_pop = 1
                else:
                    output_pop = self.edges_ports[
                                      switch][path[i-1]]['out_port']
                action = [parser.OFPActionOutput(int(output_pop))]
                self.add_flow(datapath=datapath, priority=4096, match=match,
                              actions=action)
        ...
        self.logger.info("<enable_inet_pop> finished")
