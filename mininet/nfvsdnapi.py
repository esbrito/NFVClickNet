...
class SFC(object):
    ...
    def __enable_internet(self):
        pops = self.nfs_pops.values()
        pops_location = [str(pop.location) for pop in pops]
        pop_list = ",".join(pops_location)
        url = "curl -i http://localhost:8080/sfcforwarding/inet-pops=" +\
              pop_list
        if subprocess.Popen(url, shell=True,
                            stdout=subprocess.PIPE).wait() == 0:
